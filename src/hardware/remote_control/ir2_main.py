...
def setup():
    # Use gpiod to set up the pin for input
    global line
    chip = gpiod.Chip('gpiochip0')
    line = chip.get_line(ir_pin)
    line.request(consumer='ir_sensor', type=gpiod.LINE_REQ_DIR_IN)

def binary_aquire(duration):
    # aquires data as quickly as possible
    t0 = time()
    results = []
    while (time() - t0) < duration:
        results.append(line.get_value())
    print(results)
    return results

def on_ir_receive(bouncetime=100):
    # when edge detect is called (which requires less CPU than constant
    # data acquisition), we acquire data as quickly as possible
    data = binary_aquire(bouncetime/1000.0)
    oldval = 1
    lastindex = 0
    index = 0
